...
bind pub - !def learn_learn
bind pub - !define learn_learn
bind pub - !definition learn_learn

bind pub - "\?\?" learn_explain
##bind msg - "\?\?" learn_explain
bind pub - "\*\*" learn_search
##bind msg - "\*\*" learn_search
...
proc learn_learn { nick uhost hand chan args } {
 ...
 set args [split $args " "]
 putlog "!learn ${args} for ${nick}.."
 switch [lindex $args 0] {
  "add" {
   if {[lindex $args 2] == ""} {
    puthelp "NOTICE $nick :Try !learn add word definition!" 
   } else {
      learn_addEntry $nick [lindex $args 1] [lrange $args 2 end]
      puthelp "NOTICE $nick :definition added"
  ...
  "del" {
   if {[lindex $args 2]!=""} {
    ...
    puthelp "NOTICE $nick :definition removed"
    learn_flood "[lindex $args 1]" $chan
   } else {
  ...
  "ins" {
   if {[lindex $args 3]!=""} {
    ...
    puthelp "NOTICE $nick :definition inserted"
    learn_flood "[lindex $args 1]" $chan
  ...
  default { puthelp "NOTICE $nick :Syntax: \002!learn <add|del (delete)|info (help)|ins (insert)|rep (replace)>" }
 }
exec sh /home/stefan/eggdrop/scripts/updateVerrbotWiki.sh
}
...
proc learn_explain { nick uhost hand chan args } {
	...
	putlog "!learn learn_explain for ${explain} for ${nick}"
...
proc learn_flood {word target} {
	...
	# putlog "!learn learn_fllod for ${word} on ${target}"
	global learn_whodid
...
proc learn_addEntry { nick word defenition } {
 putlog "!learn learn_addEntry '${word}' to '${defenition}' for ${nick}"
